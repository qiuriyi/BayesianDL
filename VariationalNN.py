...
class HNN:
	...
	def create_model(
		...
		model.compile(
			...
			loss_weights={'logits_variance': logits_variance_weight, 'softmax_output': 1.}
			)
...
class NN:
	...
	def create_model(
		...
		logits = tf.keras.layers.Dense(2)(logits)
		softmax_output = tf.keras.layers.Activation(activation=tf.nn.softmax,name='softmax_output')(logits)
		model = tf.keras.Model(inputs=input_tensor, outputs=softmax_output)
